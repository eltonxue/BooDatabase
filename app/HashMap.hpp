...
public:
    ...
    unsigned int bucketSize(const unsigned int index) const; // gets the size of bucket with given index

    bool equalTo(const HashMap& hm, const std::string& key) const; // checks if two HashMap classes are equal to one another

    unsigned int returnHasherValue(const std::string& key) const; // returns the value of the hasher function with given key

    void clearMainBuckets(); // clears the main Buckets

    void setNewBuckets(); // sets up new Buckets based on bucket count

    void reHashMap(); // conducts rehasing when load factor exceeds 0.8
...
private:
    ...
    unsigned int totalBucketCount;
    unsigned int totalSize;

    Node* copyLinkedNode(const Node* node); // copies linked nodes

    void clearBuckets(Node* buckets, unsigned int count); // clear the buckets given with the bucket count

    void setNullptrs(); // used after clearing all Buckets. Reinitialises the nullptrs to prevent uninitialised --memcheck error

    unsigned int findIndex(unsigned int hashValue) const; // finds the index of Buckets with given hash value
